...
## 已完成 (v2.0 → v2.1.0 → v2.2.0 → v2.3.0 → v2.5 → v2.6 → v2.7)
...
**v2.7（2026-08-18）已交付 —— UX 收尾**：远程列表/连接异步化（QtConcurrent + 代际令牌，慢速/断网目录读取不冻结 UI，状态点 Connecting 灰闪）、面板源选择器（本地/FTP/SFTP 独立浏览，右面板与工具栏双向联动）、系统文件拖入上传恢复、顺带项（readdir 错误上报 / sort 降序 SWO 修复 / 双主题像素验证测试）。测试目标扩至 13 个（新增 tst_panel_async / tst_qss_pixels）。

近期候选：

| 项目 | 优先级 | 说明 |
|------|--------|------|
| **SCP 部署上传** | 🔴 高 | SCP 无目录列表，仅部署上传场景（技术路径：libcurl scp://），集成到 FTP 双栏部署面板（v2.6 说明中预留，待补） |
| **传输可靠性** | 🔴 高 | 异步传输（系统拖入/面板拖拽上传不再卡 UI）、部署按钮连接窗口期守卫、远程面板断线重试入口 |
| **轻量化** | 🔴 高 | 安装包（NSIS/MSI）、体积优化、启动速度 |
| **单元测试扩展** | 中 | 从既有 13 目标扩展到 FtpAdapter/TelnetAdapter/ToolRegistry 覆盖率 |
