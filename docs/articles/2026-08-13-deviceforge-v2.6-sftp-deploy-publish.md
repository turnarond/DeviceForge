...
> **发布说明（主稿，CSDN/知乎/掘金通用）**
> - 底稿来源：`docs/articles/2026-08-07-deviceforge-v2.6-sftp-deploy.md`（2026-08-07 内测稿，已按 v2.6 发布后实态校正）
> - 发布日期：2026-08-13
> - 校正点：测试目标 11 个（非 8 个）、暗/亮双主题（非仅深色）、拖拽为面板间拖拽（系统文件拖入已移除）、含双栏 TC 化说明
> - **各平台发布时微调**：
>   - CSDN：标题改「Qt 6 + C++17 实现 SFTP 批量部署：工业设备运维工具实战解析」（SEO 关键词）
>   - 知乎：文首加话题标签 `#技术 #Qt #C++ #工业软件 #SFTP`
>   - 掘金：标题可改「Qt 6 工业运维工具：IDeployable 接口抽象实现 FTP/SFTP 统一部署」，正文不变
> - 公众号：另见 `2026-08-13-deviceforge-v2.6-sftp-deploy-gzh.md`（精简版 + 截图占位）
...
v2.4 把 FTP 部署从表单变成了双栏文件管理器；v2.5 补完了远程文件管理并做了 SylixOS 深度适配；v2.6 把 SFTP 从「单文件传输」变成「完整部署通道」，同时双栏面板完成 TC 化（Total Commander 风格：统一表格视图、路径栏、F2/F5/F6/Tab 快捷键）。
...
- **拖拽上传同样走部署通道**：把文件拖到对面面板即上传，拖回即下载（面板间拖拽；系统文件拖入已移除，会提示「仅支持面板间拖拽」）
...
（既有目标 `tst_nrec` / `tst_updatechecker` / `tst_dpapi_crypto` / `tst_config_store` / `tst_opcua_encode` / `tst_opcua_loopback` / `tst_remote_model` / `tst_theme` / `tst_file_source` 回归全过）
...
| 层 | 技术 |
|----|------|
| UI | Qt 6.11.1 Widgets，QSS 暗/亮双主题（设置中切换） |
...
部署页现在是一对 Total Commander 风格的双栏面板：统一表格视图 + 顶部路径栏（Enter 跳转）+ 底部面包屑；F2 重命名 / F5 复制到对面 / F6 移动到对面 / Tab 切栏；右键菜单齐全（进入/新建目录/重命名/删除/复制到对面/移动到对面/复制路径/刷新）。部署目标目录 = 右侧远程面板当前路径，先右侧导航再部署。
...
> 发布前在以下环境手工验证；凭据一律用环境变量（`DF_TEST_HOST` / `DF_TEST_USER` / `DF_TEST_PASS`）或 UI 手动输入，**不写明文密码**（文中地址均已脱敏）。

### 环境

| 设备 | 说明 |
|------|------|
| 云服务器 | Linux 云主机（地址已脱敏） |
| 局域网 Ubuntu | 工控机/服务器（地址已脱敏） |
| SylixOS 真机 | 用户 PLC 设备 |

### GUI 手动步骤

1. **连接**：协议下拉框选 SFTP → 连接 → 远程面板列出目标目录内容
2. **单文件部署**：部署 1 个文件到目标目录 → 日志显示成功 → 远程面板刷新后可见
3. **文件夹递归部署**：部署 1 个含子目录的文件夹 → 远程面板确认目录结构递归创建
4. **部署前清空**：勾选「部署前清空」→ 目标目录内容被清空且**目录本身保留**
5. **取消**：上传进行中点击取消 → 传输停止且不崩溃，日志明确
6. **多设备**：2 台设备同时部署 → 各自进度/结果正确
7. **失败隔离**：失败设备（如错误密码）→ 日志明确报错，其余设备不受影响

### SylixOS 真机

- 确认 SFTP 服务可用性（SylixOS 默认走 FTP，SSH/SFTP 需设备端支持）
- 行为差异（如有）记录到项目文档，参照 SylixOS FTP 适配经验的记录方式
...
GitHub：[turnarond/DeviceForge](https://github.com/turnarond/DeviceForge)（开源，欢迎 star / issue / PR）
