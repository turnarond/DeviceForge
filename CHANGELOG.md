...
- **面板源选择器**：FileBrowserPanel 顶部新增源类型下拉（**本地/FTP/SFTP**）+ 面板级设备下拉（远程源时显示）；左面板可独立切远程浏览源（独立连接缓存 + 代际令牌 + busy 串行化，端口取设备自身、FTPS 恒关）；右面板 ↔ 工具栏协议/设备**双向联动**（blockSignals 防循环）；左面板为远程浏览源时部署被守卫拦截（提示「请切换为本地」）；「ssh」协议 id 归一化
